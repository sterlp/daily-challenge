...
void main() {
  ...
  late BoughtRewardDao boughtRewardDao;
  ...
  test("Test calcTotal with failed challenge", () async {
    ...
    await challengeDao.save(
      Challenge.full("Test 2", null, ChallengeStatus.done, 10),
    );
    result = await creditService.calcTotal();
    expect(result, 10);

    await challengeDao.save(
      Challenge.full("Test 3", null, ChallengeStatus.failed, 5),
    );
  ...
  test("Test calcTotal with done challenge", () async {
    await challengeDao.save(
      Challenge.full("Test 1", null, ChallengeStatus.done, 10),
    );
    await challengeDao.save(
      Challenge.full("Test 2", null, ChallengeStatus.open, 20),
    );

    final result = await creditService.calcTotal();
    expect(result, 10);
  });

  test("Test calcTotal with spend credits", () async {
    await challengeDao.save(
      Challenge.full("Steuer", null, ChallengeStatus.done, 20),
    );

    await boughtRewardDao.save(
      BoughtReward()
        ..cost = 1
        ..name = 'Schoki'
        ..rewardId = 1,
    ...
    await challengeDao.save(
      Challenge.full("Saugen", null, ChallengeStatus.failed, 5),
    );
