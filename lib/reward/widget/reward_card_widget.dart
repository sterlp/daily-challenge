...
class _RewardCardWidgetState extends State<RewardCardWidget>
  ...
  Future<void> _loadBoughtReward() async {
    final id = widget._reward.id;
    if (id == null) return;
    final boughtReward = await _rewardService.getLastBoughtRewardByRewardId(id);
    if (boughtReward != null && boughtReward != _boughtReward) {
      setState(() => _boughtReward = boughtReward);
    }
  }

  Future<void> _buyReward() async {
    final buy = await showBuyRewardDialog(
      context,
      widget._reward,
      widget._credit.value,
    );
    if (buy == true) {
  ...
  Future<void> _editReward() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute<Reward>(
        builder: (BuildContext context) => RewardPage(reward: widget._reward),
  ...
  Widget build(BuildContext context) {
    ...
    final reward = widget._reward;
    _attached ??= _rewardService.attach(reward);
      ...
      child: Card(
        child: ListTile(
          ...
          subtitle: _boughtReward == null
              ...
              : Text(
                  _i18n.lastPurchase(
                    _commonI18n.formatDateTime(_boughtReward!.boughtAt),
                  ),
                ),
