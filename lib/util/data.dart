...
class ParserUtil {
  ...
  static String? valueOfEnum(Object? e) => e?.toString().split('.').last;
  ...
  static T? parseEnumStringWithDefault<T>(
    List<T> enumValues,
    String? v,
    T? defaultValue,
  ) {
    if (v == null || enumValues.isEmpty) return defaultValue;

    for (final e in enumValues) {
      if (valueOfEnum(e) == v) return e;
    }
    return defaultValue;
  }

  static List<T> mapDbResult<T>(
    List<Map<String, dynamic>> dbValues,
    DbRowMapper<T> converter,
  ) {
    final result = <T>[];
