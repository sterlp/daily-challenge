...
AppContainer buildContext([Future<Database>? database]) {
  return AppContainer()
    ...
    ..addFactory(
      (_) => database == null ? DbProvider() : DbProvider.withDb(database),
    )
    ..addFactory((_) => ConfigService())
    ..addFactory<Future<Database>>(
      (rattlinger) => rattlinger.get<DbProvider>().db,
    )
    ..addFactory(
      (rattlinger) => CreditService(
        rattlinger.get<ChallengeDao>(),
        rattlinger.get<BoughtRewardDao>(),
      ),
    )
    ..addFactory(
      (rattlinger) => ChallengeDao(rattlinger.get<Future<Database>>()),
    )
    ..addFactory(
      (rattlinger) => ChallengeService(
        rattlinger.get<ChallengeDao>(),
        rattlinger.get<CreditService>(),
      ),
    )
    ..addFactory((rattlinger) => RewardDao(rattlinger.get<Future<Database>>()))
    ..addFactory(
      (rattlinger) => BoughtRewardDao(rattlinger.get<Future<Database>>()),
    )
    ..addFactory(
      (rattlinger) => RewardService(
        rattlinger.get<RewardDao>(),
        rattlinger.get<BoughtRewardDao>(),
        rattlinger.get<CreditService>(),
      ),
    )
    ..addFactory(
      (rattlinger) => HistoryService(
        rattlinger.get<RewardService>(),
        rattlinger.get<ChallengeService>(),
      ),
    )
